...
public class UnorderedLinkedListInt extends LinkedListIntClass {
    ...
    public int findMin()  {
        ...
        min = current.info;
        while (current != null) {
            if (min > current.info) min = current.info;
            current = current.link;
        }
        return min;
    }
    ...
    public String toString() {
        ...
        LinkedListNode current = first;
        while (current != null) {
            commaList = commaList.concat(Integer.toString(current.info));
            if (current.link != null) commaList = commaList.concat(", ");
            current = current.link;
        }
        commaList = commaList.concat("]");
        return commaList;
    }
}
